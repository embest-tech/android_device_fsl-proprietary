   ...
   /******************************test alpha blending with Porter-Duff modes ***************************************/
    ...
    for(i=0; i<test_height; i++)
    {
         for(j=0; j<test_width; j++)
         {
             ...
             p = (char *)(((int)d_buf->buf_vaddr) + (i*test_width+j)*4);
    ...
    //Global Alpha: alpha blending mode G2D_DST_ALPHA, G2D_ZERO
    //set test data in src buffer
    for(i=0; i<test_height; i++)
    {
         for(j=0; j<test_width; j++)
    ...
    for(i=0; i<test_height; i++)
    {
         for(j=0; j<test_width; j++)
         {
             ...
             Co = (unsigned char)(((unsigned int)Cs * Ad * src.global_alpha) / (255*255));
             Ao = (unsigned char)(((unsigned int)As * Ad * src.global_alpha) / (255*255));
...
/****************************************** test g2d_clear *********************************************************/
    ...
    {
        int clrcolor = *(int *)(((int)d_buf->buf_vaddr) + i*4);
...
/****************************************** test g2d rotation *********************************************************/
    ...
    for(i=0; i<test_height; i++)
    {
         for(j=0; j<test_width; j++)
         {
             *(int *)(((int)s_buf->buf_vaddr) + (i*test_width+j)*4) = i*test_width+j;
         }
    }
    ...
    for(i=0; i<test_width; i++)
    {
         for(j=0; j<test_height; j++)
         {
             ...
             int rotated_val = *(int *)(((int)d_buf->buf_vaddr) + (i*test_height+j)*4);
...
/****************************************** test g2d resize performance *********************************************************/
    ...
    printf("g2d resize with vg time %dus, %dfps, %dMpixel/s ........\n", diff, 1000000/diff, test_width * test_height / diff);
    ...
    printf("g2d resize with 2d time %dus, %dfps, %dMpixel/s ........\n", diff, 1000000/diff, test_width * test_height / diff);
